...
WHERE 
    ...
    #2. Show the name and phone number of each customer.
    SELECT 
    customerName,
    phone
FROM customers;
...
#3. Count the number of orders placed in 2022.
SELECT 
    COUNT(*) AS total_orders_2022
FROM orders
...
#4. Display the names of product lines and the number of products in each line.
SELECT 
    pl.productLine,
...
SELECT 
    ...
    (SELECT COUNT(*) 
     FROM products p 
     WHERE p.productLine = pl.productLine) AS total_products
FROM productlines pl;

#5. List the first 10 orders sorted by order date.
SELECT
orderNumber,
orderDate,
status,
...
#6. Show all products priced between $50 and $100.
SELECT 
    productCode,
    productName,
    buyPrice
...
use classicmodels;
#7. Find the office with the highest number of employees.
SELECT 
    o.officeCode,
    o.city,
...
HAVING COUNT(e.employeeNumber) = (
    ...
    FROM (
        SELECT COUNT(employeeNumber) AS emp_count
        FROM employees
        GROUP BY officeCode
    ) AS office_counts
);

#8. Display the names of all customers in a specific city.
...
use classicmodels;
#Count the distinct number of product lines.
SELECT 
    COUNT(DISTINCT productLine) AS total_product_lines
FROM products;
...
#10List the details of employees who report to manager number 1056.
SELECT 
    employeeNumber,
    firstName,
    lastName,
...
WHERE m.employeeNumber = 1056;

#11. Calculate the average credit limit of customers in each city.
SELECT 
    city,
...
SELECT 
    ...
    (SELECT AVG(c2.creditLimit)
     FROM customers c2
     WHERE c2.city = c1.city) AS avg_credit_limit
FROM customers c1;

#12. Show the total amount of sales for each product line.
SELECT
p.productLine,
...
ON od.productCode = p2.productCode
WHERE p2.productLine = p1.productLine) AS total_sales
FROM products p1
GROUP BY productLine;

#13. List the customers and the total number of orders placed by each.
SELECT
c.customerNumber,
c.customerName,
...
#14. Display the top 10 most expensive products and their product lines.
SELECT
productName,
productLine,
buyPrice
...
#15. Find the average quantity ordered for each product.
SELECT
productCode,
...
(SELECT AVG(od2.quantityOrdered)
FROM orderdetails od2
WHERE od2.productCode = od1.productCode) AS avg_quantity_ordered
FROM orderdetails od1;

#16. Show the order number and total order amount for each order.
SELECT
 orderNumber,
 ...
 FROM orderdetails od2
 WHERE od2.orderNumber = od1.orderNumber) AS total_order_amount
 FROM orderdetails od1;
 
#17. List the top 5 customers based on the total amount of payments made.
SELECT
c.customerNumber,
c.customerName,
...
#18. Display the employees and the number of orders processed by each.

SELECT
e.employeeNumber,
e.firstName,
e.lastName,
COUNT(o.orderNumber) AS total_orders_processed
FROM employees e
...
INNER JOIN orders o
ON c.customerNumber = o.customerNumber
WHERE c.salesRepEmployeeNumber = e.employeeNumber) AS total_orders_processed
FROM employees e;



#19. Show the product name and the number of times it has been ordered.
SELECT
p.productName,
...
(SELECT COUNT(*)
FROM orderdetails od
WHERE od.productCode = p.productCode) AS times_ordered
FROM products p;

#20. Find the total payment amount received in each year.
SELECT
...
SELECT
DISTINCT YEAR(paymentDate) AS payment_year,
(SELECT SUM(p2.amount)
FROM payments p2
WHERE YEAR(p2.paymentDate) = YEAR(p1.paymentDate)) AS total_payment_amount
FROM payments p1;
...
select productcode,productname,quantityinstock from products where quantityinstock=0;
select productcode,productname,quantityinstock from products where case when quantityinstock=0 then 'out' else 'in' end= 'outof stkc' ;


select pl.productline, count(p.productCode) as total_products from 
productlines pl inner join products p on pl.productline=p.productline group by pl.productline;

select orderNumber from orders order by orderDate limit 10;

select productCode,productName from products where buyPrice between 50 and 100;

use classicmodels;
select o.officeCode,e.employeeNumber from offices o inner join employees e where max(count(e.employeeNumber)) group by o.officeCode;

select count(distinct productLine) from products;

select city,avg(creditLimit)  as average from customers group by city;


select c.customerNumber,count(o.orderNumber) from customers c left join orders o on c.customerNumber=o.customerNumber group by customerNumber;


#21. Rank the products based on the total quantity sold.
select p.productName,sum(od.quantityOrdered) as total_quantity_sold,
rank() over(order by sum(od.quantityOrdered) desc) as product_rank from products p inner join orderdetails od
on p.productCode=od.productCode group by p.productName;

SELECT
productName,
total_quantity_sold,
(SELECT COUNT(DISTINCT total_quantity_sold) + 1
FROM (
SELECT SUM(quantityOrdered) AS total_quantity_sold
FROM orderdetails
GROUP BY productCode
) AS t2
WHERE t2.total_quantity_sold > t1.total_quantity_sold) AS product_rank
FROM (
SELECT
p.productName,
...
#22. Using a CTE, list the average order value for each customer.
...
#23. Show the month-over-month change in total sales amount.
...
curr.sales_month,
curr.total_sales,
curr.total_sales - prev.total_sales AS mom_change
FROM (
SELECT
DATE_FORMAT(o.orderDate, '%Y-%m') AS sales_month,
SUM(od.quantityOrdered * od.priceEach) AS total_sales
...
ON prev.sales_month = DATE_FORMAT(DATE_SUB(STR_TO_DATE(CONCAT(curr.sales_month, '-01'), '%Y-%m-%d'), INTERVAL 1 MONTH), '%Y-%m');


#24. List the customers who have not made any payments.
...
use classicmodels;
#25. Display the total number of products sold in each month of 2023.
SELECT
...
#26 Rank employees based on the total sales amount they generated.
SELECT
e.employeeNumber,
...
#27. Using window functions, calculate the running total of products sold per month.
SELECT
year,
month,
monthly_products_sold,
SUM(monthly_products_sold) OVER (ORDER BY year, month) AS running_total_products_sold
FROM (
SELECT
YEAR(o.orderDate) AS year,
MONTH(o.orderDate) AS month,
SUM(od.quantityOrdered) AS monthly_products_sold
FROM orders o
JOIN orderdetails od
ON o.orderNumber = od.orderNumber
...
productLine,
AVG(buyPrice) AS average_price,
RANK() OVER (ORDER BY AVG(buyPrice) DESC) AS rank_num
FROM products
GROUP BY productLine
) ranked_data
WHERE rank_num = 1;
...
COUNT(customerNumber) - LAG(COUNT(customerNumber))
OVER (ORDER BY year) AS growth_in_customers
FROM (
SELECT
customerNumber,
...
year,
total_customers,
total_customers - LAG(total_customers) OVER (ORDER BY year) AS growth_in_customers
FROM (
SELECT
...
ORDER BY COUNT(orderNumber)) AS rank_num
FROM orders
...
SELECT 
    ...
    city,
    total_customers,
    RANK() OVER (ORDER BY total_customers DESC) AS office_rank
FROM office_customer_count;
...
use classicmodels;
#34 . List the customers and their most frequently ordered product.
SELECT
customerNumber,
customerName,
productCode,
...
FROM (
SELECT
SUM(od2.quantityOrdered) AS total_qty
FROM orders o2
JOIN orderdetails od2
ON o2.orderNumber = od2.orderNumber
WHERE o2.customerNumber = customer_products.customerNumber
GROUP BY od2.productCode
) AS max_table
)
ORDER BY customerNumber;

#35 Show the total sales amount for each quarter of each year.
SELECT
YEAR(o.orderDate) AS year,
...
#36. Using window functions, calculate the month-over-month percentage change in total Sales.
WITH monthly_sales AS (
SELECT
YEAR(o.orderDate) AS year,
MONTH(o.orderDate) AS month,
...
productLine,
total_orders,
RANK() OVER (ORDER BY total_orders DESC) AS productline_rank
FROM productline_orders;
...
productLine,
total_sales,
AVG(total_sales) OVER (PARTITION BY productLine) AS avg_line_sales
FROM product_sales
) ranked_data
WHERE total_sales < avg_line_sales
ORDER BY productLine, total_sales;
...
FROM (
SELECT
SUM(od2.quantityOrdered * od2.priceEach) AS line_sales
FROM products p2
JOIN orderdetails od2
ON p2.productCode = od2.productCode
WHERE p2.productLine = ps.productLine
GROUP BY p2.productCode
) line_avg
)
ORDER BY ps.productLine, ps.total_sales;

#39
WITH office_orders AS (
SELECT
o.officeCode,
COUNT(ord.orderNumber) AS total_orders
FROM offices o
...
officeCode,
total_orders,
ROUND((total_orders * 100.0 / total_company_orders), 2) AS percentage_of_total_orders
FROM (
SELECT
o.officeCode,
COUNT(ord.orderNumber) AS total_orders,
...
FROM offices of2
JOIN employees e2 ON of2.officeCode = e2.officeCode
